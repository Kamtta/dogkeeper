<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
  <head>
    <meta charset="utf-8" />
    <title>杜宾犬介绍</title>
    <link type="text/css" rel="stylesheet" href="css/jieshao.css" />
  </head>
  <body background="img/dogintroduction/背景图.jpg">
    <div class="nav">
      <ul>
        <li><a href="firstpage.jsp">首页</a></li>
        <li><a href="gougourichangxunlian.jsp">狗狗日常训练</a></li>
        <li><a href="gougourichnaghuli.jsp">狗狗日常护理</a></li>
        <li><a href="gougoujibingjiuzhi.jsp">狗狗疾病救治</a></li>
        <li><a href="gougouquwen.jsp">狗狗趣闻</a></li>
        <li><a href="gougouyongpinjieshao.jsp">狗狗用品介绍</a></li>
        <li><a href="gougoumengzhao.jsp">狗狗萌照</a></li>
        <li><a href="xinshouxuzhi.jsp">新手需知</a></li>
        <li><a href="hudongshequ.jsp">互动社区</a></li>
      </ul>
    </div>
    <div>
      <h1>
        <img src="img/dogintroduction/杜宾犬.jpg" align="left" width="20%" />
        <p>杜宾犬<br /></p>
        <h2>
          <span
            >杜宾犬，原产德国，它是根据培育这一品系人的名字路易斯.杜伯尔曼先生命名的，是所有品系中极富智慧的并且身体结构最为优秀，
            气质最为高贵的一种。杜宾犬是军、警两用的犬只，同时国内也有部分人将未裁耳、断尾的杜宾当做猎犬使用。
            但由于杜宾犬的体型过大，不适合在人口密集的城市中饲养，所以被很多城市列为禁养犬种。</span
          >
        </h2>
      </h1>
    </div>
    <p>
      &nbsp&nbsp&nbsp&nbsp姓名：杜宾犬&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp原产地：德国&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
      寿命：10-14年&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
      <br /><br />&nbsp&nbsp&nbsp&nbsp饲养：好养&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
      &nbsp&nbsp&nbsp&nbsp价格：500-3000元&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp功能：工作犬<br /><br />
      &nbsp&nbsp&nbsp&nbsp掉毛：适中&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp爱叫：较低频&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp干净：较干净&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
      训练：中等强度&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp综合：适中
      &nbsp&nbsp&nbsp&nbsp
    </p>
  </body>
</html>
